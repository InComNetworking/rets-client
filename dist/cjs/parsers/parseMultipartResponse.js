...
async function parseMultipartResponse(body, headers) {
    const encoding = (0, detectContentEncoding_1.detectContentEncoding)(headers);
    ...
    for (let i = -1, length = body.length; i < length;) {
        ...
        if (nextBoundayIndex === headerEndIndex + PART_SPLITTER.length) { // CREA DDF的错误XML是Header的一部分
            ...
            result.push(await (0, parseObjectResponse_1.parseObjectResponse)(headerItems[headerItems.length - 1], Object.assign(Object.assign({}, headers), (0, processHeaders_1.processHeaders)((0, lodash_flatten_1.default)(headerItems.slice(0, headerItems.length - 1).map(v => [v.substring(0, v.indexOf(':')), v.substring((v.indexOf(':') + 1)).trim()]))))));
            i = nextBoundayIndex;
        }
        else { // 正常返回，或其他情况
            nextBoundayIndex -= LINE_SPLITTER.length;
            const content = body.slice(headerEndIndex + PART_SPLITTER.length, nextBoundayIndex);
            result.push(await (0, parseObjectResponse_1.parseObjectResponse)(content, Object.assign(Object.assign({}, headers), (0, processHeaders_1.processHeaders)((0, lodash_flatten_1.default)(headerText.split('\r\n').map(v => [v.substring(0, v.indexOf(':')), v.substring((v.indexOf(':') + 1)).trim()]))))));
...
function findBoundary(headers) {
    const boundary = (0, defaultValue_1.defaultValue)(headers.ContentType).match(/boundary=([^;]+)/);
    if (!boundary) {
        throw new models_1.RetsProcessingError(new TypeError('Could not find boundary under Content-Type'));
    }
    return `--${(0, lodash_trim_1.default)(boundary[1], '"')}`;
}
