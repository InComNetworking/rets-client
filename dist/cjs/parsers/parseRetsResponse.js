...
async function parseRetsResponse(source, recordXmlTagName, ignoreRepliedError = false) {
    ...
    const result = {
        ...
        statusMessage: (0, ReplyCode_1.findReplyCodeName)(root[XML_ATTRIBUTES].ReplyCode) || 'Unknown reply code',
        extra: {}
    };
    if (root['RETS-STATUS']) { // 额外状态
        ...
        result.statusMessage = (0, ReplyCode_1.findReplyCodeName)(result.replyCode) || 'Unknown reply code';
    }
    if (root['COLUMNS']) { // COMPACT数据
        let dataDelimiter = '\t';
        if (root['DELIMITER']) {
            dataDelimiter = (0, decodeHexString_1.decodeHexString)(root['DELIMITER'][XML_ATTRIBUTES].value);
        }
