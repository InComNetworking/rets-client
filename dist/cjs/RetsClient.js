...
class RetsClient {
    ...
    constructor(configuration) {
        ...
        this.actions = {};
        this.headers = {};
        this.configuration = (0, lodash_clonedeep_1.default)(configuration);
        this.createHeader();
        this.session = (0, request_1.defaults)({
            jar: (0, request_1.jar)(),
    ...
    async login() {
        ...
        source.split('\r\n').filter(v => v.indexOf('=') > -1).map(v => v.replace(/\s/g, '').split('=')).forEach(url => {
            ...
            if (action != null) {
                this.actions[action] = this.session.defaults({ uri: (0, replaceAddress_1.replaceAddress)(address, this.configuration.url) });
            }
        });
    }
    /**
    ...
    async search(options) {
        const response = await this.sendAction(this.findRequest(models_1.RetsAction.Search), (0, combine_1.combineQueryOptions)(options));
        if (response instanceof Error) {
            throw response;
        }
    ...
    async getObjects(options) {
        ...
        const response = await this.sendAction(action, (0, combine_1.combineObjectOptions)(options));
        if (response instanceof Error) {
            throw response;
        }
    ...
    createHeader() {
        ...
        if (this.configuration.userAgentPassword) {
            this.headers['RETS-UA-Authorization'] = 'Digest ' + (0, crypto_1.createHash)('md5').update([
                (0, crypto_1.createHash)('md5').update(`${this.configuration.userAgent}:${this.configuration.userAgentPassword}`).digest('hex'),
                '',
    ...
    async sendAction(action, query) {
        ...
        if (data.response.statusCode !== 200) {
            ...
            const headers = (0, processHeaders_1.processHeaders)(data.response.rawHeaders);
            let body;
            if ((0, isIncluded_1.isIncluded)(v => v.includes('text/xml'), headers.ContentType)) {
                body = (0, parseRetsResponse_1.parseRetsResponse)(data.body);
            }
            else if ((0, isIncluded_1.isIncluded)(v => v.includes('multipart'), headers.ContentType)) {
                body = (0, parseMultipartResponse_1.parseMultipartResponse)(data.body, headers);
            }
            else {
                body = (0, parseObjectResponse_1.parseObjectResponse)(data.body, headers);
            }
            const response = {
                headers: headers,
